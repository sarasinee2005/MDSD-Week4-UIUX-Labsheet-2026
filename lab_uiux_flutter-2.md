...
| สี | หน้าที่ |
|-----|--------|
| Primary | สีหลักของแอป สำหรับปุ่มสำคัญที่สุด หรือแอปบาร์ |
| On Primary | สีข้อความ/ไอคอนที่อยู่ บนสี Primary |
| Primary Container | สีพื้นหลังการ์ดหรือกล่องข้อความที่เน้นความสำคัญรองลงมา |
| On Primary Container | สีข้อความ/ไอคอนที่อยู่ บนสี Primary Container |
...
<img width="1295" height="882" alt="image" src="https://github.com/user-attachments/assets/54beac3d-b561-4abb-89eb-50011ea3aa99" />

<img width="1301" height="882" alt="image" src="https://github.com/user-attachments/assets/8108bda2-6f1d-45f2-af89-b40171d534e7" />
...
| AI handle กรณี avatarUrl เป็น null อย่างไร? | แสดงตัวอักษรย่อของชื่อ (Initials) บน primaryContainer |
| AI ใช้ color จาก Theme หรือ hardcode? | ใช้จาก Theme (Theme.of(context).colorScheme) |
| มีส่วนไหนที่ควรปรับปรุง? | ปรับให้รองรับรูปโปรไฟล์จากไฟล์เครื่อง/Asset |
...
```
จากการเปรียบเทียบภาพโครงร่างต้นแบบ (Wireframe) และผลลัพธ์ของแอปพลิเคชันที่พัฒนาขึ้นจริง พบว่าโครงสร้างการจัดวางองค์ประกอบส่วนใหญ่มีความสอดคล้องกันอย่างเหมาะสม
แต่มีข้อแตกต่างสำคัญในเรื่องของการเลือกใช้ระบบสี ความชัดเจนของข้อความ และองค์ประกอบด้านการนำทาง โดยในส่วนของหน้าหลัก (Home Screen) แอปพลิเคชันที่เขียนขึ้นจริงสามารถจัดวางกล่องรายการสินค้า
ปุ่มเพิ่มข้อมูล และหัวข้อบนแอปบาร์ได้ตรงตามแบบ แต่ยังขาดส่วนของแถบเมนูด้านล่าง (Bottom Navigation Bar) ซึ่งทำให้การสลับหน้าจอไม่สมบูรณ์เท่าต้นแบบ สำหรับหน้ารายละเอียดสินค้า (Detail Screen)
แม้จะมีการวางโครงสร้างภาพ รายละเอียด และปุ่มกดได้อย่างครบถ้วน รวมถึงมีการเพิ่มป้ายกำกับพิเศษเข้ามา แต่จุดที่ต้องปรับปรุงอย่างเร่งด่วนในทุกหน้าจอคือเรื่องของความตัดกันของสี (Color Contrast)
เนื่องจากตัวอักษรหลายจุด เช่น ชื่อสินค้า รายละเอียด และชื่อผู้ใช้ในหน้าโปรไฟล์ (Profile Screen) ถูกกำหนดให้เป็นสีสว่างที่กลืนไปกับพื้นหลังสีอ่อน ทำให้มองเห็นได้ยากและอ่านลำดับข้อมูลได้ลำบาก
จึงควรแก้ไขโดยการกำหนดสีตัวอักษรให้เข้มขึ้นเพื่อให้แสดงผลได้อย่างชัดเจนและใช้งานได้จริงตามมาตรฐานการออกแบบ
```
...
```
คำตอบ: Material 3 เปลี่ยนจากการใช้สีเดี่ยวแบบตั้งค่าแยกชิ้นใน Material 2 มาเป็นการใช้ Dynamic Color System และ Tonal Palettes (Color Schemes/Tokens)
ที่สร้างชุดสีที่มีความเข้ากันและคำนึงถึงค่าความสว่าง-ความเข้ม (Contrast) โดยอัตโนมัติ ทำให้การจัดการธีม Light/Dark Mode และการคุมโทนสีทั่วทั้งแอปทำได้ยืดหยุ่นและเป็นระบบมากขึ้น
```

**ข้อ 2:** เมื่อแปลง Figma Design เป็น Flutter Widget พบปัญหาอะไรบ้าง และแก้ไขอย่างไร?

```
คำตอบ: พบปัญหาเรื่อง การกำหนดขนาด/ตำแหน่งที่ตายตัวเกินไป (Hardcoded Dimensions), ความไม่สมดุลของสีและข้อความ (Color Contrast/Theme Mismatch) ที่ทำให้ข้อความกลืนกับพื้นหลัง และ ส่วนประกอบบางชิ้นใน UI ขาดหายไป (เช่น BottomNavigationBar); แก้ไขโดยการ refactor โค้ดให้ดึงค่าสีจาก Theme.of(context).colorScheme, ปรับ Layout ให้เป็น Responsive ด้วย Widget ยืดหยุ่น เช่น Expanded/Padding และเพิ่ม Component ที่ขาดหายไปตามต้นแบบ
```

**ข้อ 3:** Code ที่ AI สร้างให้นั้นสมบูรณ์แค่ไหน? ต้องปรับปรุงอะไรบ้าง?

```
คำตอบ: โค้ดที่ AI สร้างให้มีความสมบูรณ์ในระดับ โครงสร้างพื้นฐานและ Layout (ประมาณ 70-80%) ทำให้เห็นภาพรวมได้เร็ว แต่สิ่งที่ต้องปรับปรุงคือการจัดการเกี่ยวกับ Color Contrast (สีตัวอักษรกลืนกับพื้นหลัง),
การทำ State Management สำหรับการเปลี่ยนหน้าหรือโต้ตอบจริง รวมถึงการทำ Data Handling และจัดการกรณีข้อมูลเป็น null ให้ครอบคลุมยิ่งขึ้น
```

**ข้อ 4:** ถ้าจะนำ UI ที่ออกแบบไปใช้กับ Project จริง จะปรับปรุงอะไรบ้าง?

```
คำตอบ: ปรับปรุงโดยการ 1) แยกโครงสร้างโค้ดและ Widget ออกเป็นไฟล์ย่อย (Modularization) เพื่อให้บำรุงรักษาง่าย 2) เชื่อมต่อ State Management (เช่น Provider, Bloc หรือ Riverpod) เข้ากับระบบข้อมูลจริง/API 3) แก้ไขเรื่อง Accessibility/Contrast ของสีและฟอนต์ให้อ่านง่ายในทุกสภาวะแสง และ 4) เพิ่มระบบ Navigation ที่สมบูรณ์ เช่น แถบเมนูด้านล่างและการเปลี่ยนหน้าแบบราบรื่น
```
